...
const ClientVideosSection: React.FC = () => {
  ...
  const getVisibleVideos = () => {
    ...
    if (videos.length === 2) {
      ...
      for (let i = 0; i < 2; i++) {
        const index = (activeIndex + i) % videos.length;
        visible.push({
          ...videos[index],
          position: i,
          originalIndex: index,
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-16 lg:py-24 bg-[#01292B] overflow-hidden"
    >
      {/* Background Elements */}
      ...
      <div className="container font-cinzel relative z-20 max-w-7xl px-4">
        ...
        >
          <button className="group relative inline-flex items-center gap-2 px-8 py-3 border-2 border-[#CDB04E] text-[#CDB04E] rounded-full font-josefin font-semibold hover:bg-[#CDB04E] hover:text-black transition-all duration-300">
            View More
