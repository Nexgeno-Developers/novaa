...
export default function ProjectTabsSection({ project }: ProjectTabsProps) {
  ...
  const handleTabChange = (tabId: string) => {
    ...
    setActiveTab(tabId);
    setShowAllItems(false); // Reset to show limited items when switching tabs
  };
  ...
  // Get items to display based on showAllItems state
  const getItemsToDisplay = () => {
    if (!activeTabData) return [];

    if (showAllItems) {
      return activeTabData.items.sort((a, b) => a.order - b.order);
    } else {
      return activeTabData.items.sort((a, b) => a.order - b.order).slice(0, 6);
    }
  };
  ...
  // If no tabs exist, don't render the component
