...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch footer data', details: errorMessage }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return NextResponse.json({ error: 'Failed to update footer data', details: errorMessage }, { status: 500 });
  }
}