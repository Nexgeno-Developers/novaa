...
export async function GET(request: NextRequest) {
  try {
    ...
    const resource_type =
      resourceTypeParam && resourceTypeParam !== "all"
...
export async function POST(request: NextRequest) {
  try {
    ...
    // UPDATED: Expanded allowed file types to include documents and SVG
    const allowedTypes = [
      // Images
      "image/jpeg",
      "image/jpg",
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json(
        {
          success: false,
          error: `File type ${file.type} is not allowed. Supported types: Images (including SVG), Videos, PDF, Word, Excel, PowerPoint, Text files.`,
        },
        { status: 400 }
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const success = await cloudinaryService.deleteFile(
      public_id,
      resourceTypeToUse
    );
