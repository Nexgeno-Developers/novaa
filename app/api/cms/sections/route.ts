...
export async function POST(request: NextRequest) {
  try {
    ...
    // Add delay to ensure DB write is complete
    await new Promise(resolve => setTimeout(resolve, 500));

    // Revalidate the appropriate paths
    try {
      if (sectionData.pageSlug === 'home') {
        revalidatePath('/', 'page');
      } else if (sectionData.pageSlug === 'project') {
        revalidatePath('/project', 'page');
      } else {
        revalidatePath(`/${sectionData.pageSlug}`, 'page');
      }
      
      revalidateTag("sections");
      revalidateTag(`${sectionData.pageSlug}-sections`);
      
      console.log('New section created and paths revalidated:', {
        section: sectionData.slug,
        page: sectionData.pageSlug,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Add delay to ensure DB writes are complete
    await new Promise(resolve => setTimeout(resolve, 500));
    ...
    try {
      ...
      console.log('Section order updated and paths revalidated:', {
        page: pageSlug,
        timestamp: new Date().toISOString()
      });
    } catch (revalidateError) {
