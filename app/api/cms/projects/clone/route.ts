...
export async function POST(request: Request) {
  try {
    ...
    const clonedData = {
      ...
      projectDetail: originalProject.projectDetail
        ? JSON.parse(JSON.stringify(originalProject.projectDetail))
        : undefined,
    };
    ...
    // Remove any _id fields from nested documents
    function removeNestedIds(obj: any) {
      if (Array.isArray(obj)) {
        obj.forEach(removeNestedIds);
      } else if (obj && typeof obj === "object") {
        delete obj._id;
        for (const key in obj) removeNestedIds(obj[key]);
      }
    }
    removeNestedIds(sanitizedData);

    console.log("Creating sanitized clone...");
    const clonedProject = await Project.create(sanitizedData);

    // Create the cloned project
    // const clonedProject = await Project.create(clonedData);

    // Populate the category for the response
    const populatedClonedProject = await Project.findById(
      clonedProject._id
    ).populate("category");
    ...
    if (error.code === 11000) {
      return NextResponse.json(
        {
          success: false,
          error: "Failed to generate unique slug for cloned project",
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: `Failed to clone project ${error} ${error.message}` },
      { status: 500 }
    );
  }
}
