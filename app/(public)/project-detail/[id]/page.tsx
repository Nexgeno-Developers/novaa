...
// Create cached function for project data - matches your API cache tags
const getCachedProjectById = (id: string) =>
  unstable_cache(
    async () => {
      try {
        ...
        // Ensure projectDetail exists with defaults (matching your API structure)
        if (!updatedProject.projectDetail) {
          updatedProject.projectDetail = {
            hero: {
              backgroundImage: "",
...
// Generate static params for known projects at build time
export async function generateStaticParams() {
  try {
    await connectDB();
    const projects = await Project.find({ isActive: true })
...
// Allow dynamic params for new projects that weren't built statically
export const dynamicParams = true;
// Don't auto-revalidate - rely on your CMS API to trigger revalidation
export const revalidate = false;